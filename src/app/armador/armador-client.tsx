...
import {
  PERFILES, getPerfil, buildResumen, type ArmadorPerfil, type ArmadorSlot,
} from "@/lib/armador-perfiles";
import {
  PLATAFORMAS, ENTREGA_LABEL, placasParaPerfil, placaRecomendada, verificarCpuPlaca, socketDeCpu,
  memoriaDeCpu,
  type Plataforma, type PlacaOpcion,
} from "@/lib/armador-plataformas";
...
/** Slots ya resueltos para la plataforma elegida: los procesadores se filtran por
 *  plataforma y la placa madre se rellena con el catálogo real de esa plataforma. Es lo
 *  que impide que el cliente arme un imposible (un Core i5 con una placa AM5). */
function slotsDePlataforma(
  perfil: ArmadorPerfil,
  plataforma: Plataforma,
  placas: PlacaOpcion[],
  cpu?: string,
): ArmadorSlot[] {
  return perfil.slots.map((slot) => {
    if (slot.key === "motherboard") {
      // Las placas dependen del procesador elegido: primero su SOCKET, que es lo que
      // decide si la máquina existe, y después la gama que le corresponde al perfil
      // (un PC Gamer no se arma sobre una A520 aunque esté en las listas).
      const aptas = placasParaPerfil(placas, perfil.id, cpu ? socketDeCpu(cpu) : null);
      const rec = placaRecomendada(aptas, perfil.id);
      return {
        ...slot,
        opciones: aptas.map((p) => ({ label: p.label, entrega: p.entrega, rec: p.label === rec })),
      };
    }
    if (slot.key === "ram" && cpu) {
      // La memoria la manda el socket: un Ryzen 7000 no arranca con DDR4 y un Ryzen
      // 5000 no acepta DDR5. Se reetiquetan las capacidades del perfil en vez de
      // mantener dos listas paralelas que se desincronizan.
      const ddr = memoriaDeCpu(cpu);
      if (ddr) {
        return {
          ...slot,
          opciones: slot.opciones.map((o) => ({ ...o, label: o.label.replace(/DDR\d/, ddr) })),
        };
      }
    }
    const filtradas = slot.opciones.filter((o) => !o.plataforma || o.plataforma === plataforma);
    return filtradas.length > 0 ? { ...slot, opciones: filtradas } : slot;
  });
}

/** Selección inicial = nivel recomendado de cada slot (o el primero). */
function defaultSeleccion(slots: ArmadorSlot[]): Record<string, string> {
...
export function ArmadorClient({ placas }: { placas: Record<Plataforma, PlacaOpcion[]> }) {
  const router = useRouter();
  const [plataforma, setPlataforma] = useState<Plataforma | null>(null);
  const [step, setStep] = useState<1 | 2 | 3>(1);

  // El paso 2 es largo: al elegir los componentes el cliente termina abajo del todo, y
  // al pasar al resumen el navegador CONSERVA esa posición — la pantalla se quedaba en
  // el pie de página y "Revisa tu configuración" no se veía. Cada cambio de paso vuelve
  // al principio del armador. El margen de scroll deja libre el navbar fijo (114px en
  // móvil, 140px en escritorio); sin él, el título queda tapado por la barra.
  const inicio = useRef<HTMLDivElement>(null);
  const montado = useRef(false);
  useEffect(() => {
    // En el primer render no se toca el scroll: la página acaba de cargar arriba.
  ...
  // Los slots que se muestran dependen de la plataforma: se recalculan al cambiarla.
  const slots = useMemo(
    () => (perfil && plataforma
      ? slotsDePlataforma(perfil, plataforma, placas[plataforma], seleccion.cpu)
      : []),
    [perfil, plataforma, placas, seleccion.cpu],
  );

  // Compatibilidad: la decide el motor determinista sobre la placa REALMENTE elegida,
  // no el texto que se le manda a Andrea.
  const placaElegida = useMemo(
    () => (plataforma ? placas[plataforma].find((p) => p.label === seleccion.motherboard) ?? null : null),
    [plataforma, placas, seleccion.motherboard],
  );

  const veredicto = useMemo(
    () => (plataforma ? verificarCpuPlaca(seleccion.cpu ?? "", placaElegida, plataforma) : null),
    [seleccion.cpu, placaElegida, plataforma],
  );

  // La entrega del equipo la marca la pieza más lenta: si una sola va por encargo, el
  // conjunto se entrega en el plazo largo.
  const entrega = useMemo(
    () => (slots.some((s) =>
      s.opciones.some((o) => o.label === seleccion[s.key] && o.entrega === "encargo"),
    ) ? "encargo" : "rapida"),
    [slots, seleccion],
  );
  ...
  const resumen = useMemo(
    () => (perfil && plataforma
      ? `${buildResumen({ ...perfil, slots }, seleccion)} · Plataforma: ${plataforma.toUpperCase()}`
      : ""),
    [perfil, plataforma, slots, seleccion],
  );
  ...
  /** Al elegir plataforma se recalculan los slots y se preseleccionan los recomendados. */
  function elegirPlataforma(p: Plataforma) {
    if (!perfil) return;
    setPlataforma(p);
    // Dos pasadas: la primera da el procesador recomendado y la segunda resuelve las
    // placas de SU socket. Sin esto el equipo por defecto podría salir con un Ryzen AM5
    // y una placa AM4.
    const base = slotsDePlataforma(perfil, p, placas[p]);
    const cpu = defaultSeleccion(base).cpu;
    setSeleccion(defaultSeleccion(slotsDePlataforma(perfil, p, placas[p], cpu)));
  }

  /** Cambiar de procesador puede invalidar la placa: un Ryzen 5 5600G es AM4 y un
   *  Ryzen 5 7600 es AM5. En vez de dejar en pantalla una combinación imposible, se
   *  repone la placa por la que recomendamos para el socket nuevo. */
  function elegirOpcion(key: string, valor: string) {
    setSeleccion((prev) => {
      const next = { ...prev, [key]: valor };
      if (key !== "cpu" || !perfil || !plataforma) return next;
      const nuevos = slotsDePlataforma(perfil, plataforma, placas[plataforma], valor);
      // Placa y memoria dependen del procesador: si la elección anterior ya no encaja
      // en el socket nuevo, se repone por la que recomendamos.
      for (const key2 of ["motherboard", "ram"]) {
        const opciones = nuevos.find((s) => s.key === key2)?.opciones ?? [];
        if (opciones.length === 0 || opciones.some((o) => o.label === next[key2])) continue;
        next[key2] = (opciones.find((o) => o.rec) ?? opciones[0])?.label ?? "";
      }
      return next;
    });
  }
  ...
  return (
    <div ref={inicio} className="mx-auto max-w-5xl scroll-mt-32 px-4 py-8 sm:px-6 lg:scroll-mt-40 lg:px-8">
      {/* Breadcrumb */}
      ...
      {step === 2 && perfil && (
        <section className="animate-fade-in-up grid gap-6 lg:grid-cols-[1fr_300px]" style={{ animationDuration: "0.25s" }}>
          <div>
            ...
            {/* Plataforma: primero, porque de ella dependen procesador y placa madre */}
            <div className="mb-3 rounded-2xl border border-zinc-200 bg-white p-4">
              <div className="mb-2.5 flex items-center gap-2">
                <Cpu className="h-4 w-4 text-zinc-400" />
                <h3 className="text-sm font-semibold text-zinc-800">Plataforma</h3>
              </div>
              <div className="flex flex-wrap gap-2">
                {PLATAFORMAS.map((p) => (
                  <button
                    key={p.id}
                    onClick={() => elegirPlataforma(p.id)}
                  ...
                  >
                    ...
                    <span className="text-[11px] leading-tight opacity-70">{p.nota}</span>
                  </button>
                ))}
              </div>
              {!plataforma && (
                <p className="mt-2 text-[11px] text-zinc-500">
                  Elige la plataforma para ver los procesadores y las placas que le corresponden.
                </p>
              )}
            </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {slot.opciones.map((op) => {
                        ...
                        return (
                          <button
                            key={op.label}
                            onClick={() => elegirOpcion(slot.key, op.label)}
                          ...
                          >
                            ...
                            {op.label}
                            {op.entrega === "encargo" && (
                              <span className="ml-1 rounded-full bg-amber-50 px-1.5 py-0.5 text-[9px] font-semibold text-amber-700">
                                {ENTREGA_LABEL.encargo}
                              </span>
                            )}
                            {op.rec && (
      ...
      {step === 3 && perfil && (
        <section className="animate-fade-in-up mx-auto max-w-2xl" style={{ animationDuration: "0.25s" }}>
          ...
          {/* Compatibilidad verificada + plazo real del conjunto */}
          <div className="mt-3 space-y-2">
            {veredicto && (
              <div
                className={`flex items-start gap-2 rounded-xl border px-4 py-2.5 text-xs ${
                  veredicto.nivel === "ok"
              ...
              >
                ...
                <span>{veredicto.razones.join(" ")}</span>
              </div>
            )}
            <div className="flex items-start gap-2 rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-2.5 text-xs text-zinc-600">
              <Truck className="mt-px h-4 w-4 shrink-0 text-zinc-400" />
              <span>
                Entrega estimada: <strong className="font-semibold text-zinc-800">{ENTREGA_LABEL[entrega]}</strong>
                {entrega === "encargo" && " — por la configuración que elegiste."}
              </span>
            </div>
          </div>
