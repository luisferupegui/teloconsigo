  ...
  return (
    <Link
      ...
      className="group flex flex-col rounded-2xl border border-zinc-200 bg-white
                 ...
                 hover:-translate-y-1 hover:border-[#1e6cff]/40
...
export default async function CategoriaPage({
  ...
  const marcasMap = new Map<string, Linea[]>();
  for (const linea of cat.lineas ?? []) {
    if (!marcasMap.has(linea.marca)) marcasMap.set(linea.marca, []);
    marcasMap.get(linea.marca)!.push(linea);
  }
  const marcas = [...marcasMap.entries()];
  const totalLineas = cat.lineas?.length ?? 0;
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="flex items-center gap-5 mb-2">
        {cat.imagen ? (
          <div className="relative h-20 w-20 shrink-0 rounded-2xl overflow-hidden
                          border border-[#1e6cff]/20 bg-[#1e6cff]/5">
            <Image
              src={cat.imagen}
              alt={cat.nombre}
              fill
      ...
      {totalLineas > 0 && (
        <p className="text-xs text-zinc-400 mb-8 mt-1">
          {marcas.length} marca{marcas.length !== 1 ? "s" : ""} · {totalLineas} línea{totalLineas !== 1 ? "s" : ""}
        </p>
      )}
